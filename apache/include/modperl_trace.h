...
#define MP_VERSION_STRING "mod_perl/1.99_13"
#define MP_TRACE_OPTS "acdefghimost"
...
#endif
#ifdef MP_TRACE
#define MP_TRACE_s if (MP_debug_level & 1024) modperl_trace
#define MP_TRACE_s_do(exp) if (MP_debug_level & 1024) { \
exp; \
}
#else
...
#endif
#ifdef MP_TRACE
#define MP_TRACE_t if (MP_debug_level & 2048) modperl_trace
#define MP_TRACE_t_do(exp) if (MP_debug_level & 2048) { \
exp; \
}
#else
...
modperl_trace(NULL, " g %s (Perl runtime interaction)", (MP_debug_level & 32) ? "On " : "Off"); \
...
modperl_trace(NULL, " s %s (perl sections)", (MP_debug_level & 1024) ? "On " : "Off"); \
modperl_trace(NULL, " t %s (benchmark-ish timings)", (MP_debug_level & 2048) ? "On " : "Off");

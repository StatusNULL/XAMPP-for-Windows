...
int WINAPI WinMain(HINSTANCE hExe, HINSTANCE hPrev, LPSTR CmdLine, int CmdShow)
{
	...
	GetPrivateProfileString("START", "tomcat",  "0", tomcat_start,  64, ini_file);

	GetPrivateProfileString("EXIT", "httpd",  "0", apache_stop,   64, ini_file);
	GetPrivateProfileString("EXIT", "mysql",   "0", mysql_stop,    64, ini_file);
	GetPrivateProfileString("EXIT", "mysql",   "0", tomcat_stop,    64, ini_file);
	...
	GetPrivateProfileString("PORTS", "tomcat","8080", tomcat_port, 64, ini_file);
	...
	strcpy(job[4].name, "tomcat6");
	strcpy(job[4].port, tomcat_port);
...
// 11.2.2006 was kprintf - KAY
            kprintf("Apache \t%s \t%s\nMySql \t%s \t%s\nFilezilla \t%s \t%s\nMercury \t%s \t%s\nTomcat \t%s \t%s\n",
					...
					job[4].service?"svc":"app", job[4].state?"running":"stopped");
            return 0;
        }
    ...
    if (argc % 2 == 0) {
        kprintf("Usage: xampp [status | [stop | start module]...]\n\nmodule: %s | %s | %s | %s\n\naction: start | stop",
                job[0].name, job[1].name, job[2].name, job[3].name, job[4].name);
        return SYSERR;
    }
...
BOOL APIENTRY MyDlgProc (HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg) {
        ...
        case WM_INITDIALOG:
			...
			java_installed = GetJavaSDKState(1024, "findjava");
			if ( java_installed != 0) { 
				lbprintf(hList, "Tomcat comtrols are active: Found Java SDK on system");
			} else {
  				lbprintf(hList, "Tomcat comtrols are NOT active: Cannot found Java SDK on your system");
			}
            ...
            if (error) {
                ...
                lbprintf(hList, "WARN:This program must be run from your XAMPP root directory.");
				lbprintf(hList, "INFO:Perhaps this program running on a 64 bit plattform please ignore the message above.");
            }
            else
        ...
        case WM_COMMAND:
            ...
            switch (wmID) {
                ...
                case IDC_MYSQL:
					ShellExecute(hWnd, "open", "http://localhost/phpmyadmin/", NULL, NULL, SW_SHOWNORMAL);
                    return 0;
				...
				case IDC_TOMCAT:
					ShellExecute(hWnd, "open", "http://localhost:8080/", NULL, NULL, SW_SHOWNORMAL);
                    return 0;
                ...
                case IDC_CHECKBOX2:
                    ...
                    if (job[4].state) {
                        kprintf("ERROR: Tomcat is currently running");
                        if (job[4].service)
                            CheckDlgButton(hDlg, IDC_CHECKBOX8, BST_CHECKED);
                        else
                            CheckDlgButton(hDlg, IDC_CHECKBOX8, BST_UNCHECKED);
                        break;
                    }

                    update_flag = 0;

                    if (IsDlgButtonChecked(hDlg, IDC_CHECKBOX8) == BST_UNCHECKED) {
                        if (kprintf("Click OK to uninstall the Tomcat6 Service") == IDOK) {
                            execw("tomcat\\tomcat_service_uninstall.bat", SW_SHOW, &exit_code, 0, DETACHED_PROCESS);
                        }
                        else
                            CheckDlgButton(hDlg, IDC_CHECKBOX8, BST_CHECKED);
                    }
                    else {
                        if (kprintf("Click OK to install the Tomcat6 Service") == IDOK)
							execw("tomcat\\tomcat_service_install.bat", SW_SHOW, &exit_code, 0, DETACHED_PROCESS);
...
int UpdateStatus(HWND hDlg)
{
    char tmp[256];
    int i, result, error;
	int pidfile = file_exists("tomcat\\logs\\catalina.pid");
	...
	//	EnableButtons(hDlg, 1);
		EnableWindow(GetDlgItem(hDlg, IDC_CHECKBOX4), 1);
		EnableWindow(GetDlgItem(hDlg, IDC_ACTION8), 1);
		// EnableWindow(GetDlgItem(hDlg, IDC_TOMCATTEXT), 1);
		
	}

    // Determine FileZilla FTP SERVICE status

    result = GetServiceStatus("FileZillaServer");
	...
	// Tomcat60
	 ...
	 result = GetServiceStatus("Tomcat6");

    if (result > 0 && pidfile == 0) {
        ...
        if (result == 2) {
            SetDlgItemText(hDlg, IDC_EDIT8, "Running");
            SetDlgItemText(hDlg, IDC_ACTION8, "Stop");
            job[4].state = 1;
        }
        else {
            SetDlgItemText(hDlg, IDC_EDIT8, "Stopped");
            SetDlgItemText(hDlg, IDC_ACTION8, "Start");
            job[4].state = 0;
        }
    }
    else { 
	 ...
	 // Determine Kay SERVICE status



    	for (i=0; i<proc_index; i++) {
		if (stristr(proc_tab[i].module, "java.exe") && (pidfile == 1)) {
		// if (stristr(proc_tab[i].module, "java.exe")) {
			if (hDlg && update_flag) {
				SetDlgItemText(hDlg, IDC_EDIT8, "Running");
				SetDlgItemText(hDlg, IDC_ACTION8, "Stop");
			}
			job[4].state = 1;
			job[4].dwPID = proc_tab[i].pid;
			break;
		} 
	}
	if (i == proc_index) {
	 ...
	 // Determine Kay SERVICE status ends
	...
	EnableWindow(GetDlgItem(hDlg, IDC_TOMCAT), job[4].state);
...
int do_job(int m, int n) {
    ...
    switch (m) {
		...
		case 4:
			...
			int pidfile = file_exists("tomcat\\logs\\katalina.pid");

			//	WinExec ("catalina_start.bat", SW_SHOWNORMAL);
				result = WinExec ("catalina_start.bat", SW_HIDE);
				// if (result == 33 && pidfile == 1) { 
				if (result == 33) { 
                lbprintf(hList, "Tomcat started [Port %s]", tomcat_port);
				// lbprintf(hList, "Tomcat return code [%d]", pidfile);
				// lbprintf(hList, "Tomcat return code [%d]", result);
				}
                else {
					lbprintf(hList, "ERROR:This program must be run from your XAMPP root directory.");
                    // lbprintf(hList, "ERROR: Tomcat not started! Perhaps JAVA_HOME environment variable is not set! [%d]", result);
					// lbprintf(hList, "INFO: Perhaps Java SDK is not installed or the JAVA_HOME environment variable was not set!");
					// lbprintf(hList, "Tomcat pif file  [%s]", pidfile);
				}
            }
            else {
			//	result= WinExec ("catalina_stop.bat", SW_SHOWNORMAL);
				result= WinExec ("catalina_stop.bat", SW_HIDE);
				// Sleep(3000); 
                if (result == 33) {
                    lbprintf(hList, "Tomcat stopped [Port %s]", tomcat_port);
				}
                else {
                    lbprintf(hList, "ERROR: Tomcat not stopped [%d]", result);
					lbprintf(hList, "Tomcat return code [%s]", result);
				}
            }
            break;
    }
...
BOOL GetJavaSDKState(int len, char *path)
{

...
and load the tokenizer extension for faster parsing (your version is ".phpversion()."\n");
    ...
    function readConfigFile($file)
    {
        ...
        {
            $configdir = str_replace('\\','/', 'M:\xampp\php\pear\data/PhpDocumentor') . PATH_DELIMITER . 'user' . PATH_DELIMITER;
    ...
    function parseIni()
    {
        ...
        flush();
        $options = phpDocumentor_parse_ini_file(str_replace('\\','/', 'M:\xampp\php\pear\data/PhpDocumentor') . PATH_DELIMITER . 'phpDocumentor.ini',true);

        if (!$options)
        {
            print "ERROR: cannot open phpDocumentor.ini in directory M:\xampp\php\pear\data/PhpDocumentor\n";

...
::            for PHP 4.3 C:\phpdev\php-4.3-Win32\cli\php.exe
  ...
  SET phpCli=M:\xampp\php\cli\php.exe
...
SET count=0
FOR /R "M:\xampp\php\pear\data\PhpDocumentor\user" %%I IN (*.ini) DO (
  SET /a count+=1
...
:run
SET found=1
ECHO Starting: "%phpCli%" "M:\xampp\php\phpdoc" -c "%iniFile%"
ECHO.
"%phpCli%" "M:\xampp\php\phpdoc" -c "%iniFile%"
GOTO :EOF

...
IF '%1'=='' (
  ...
  "%phpCli%" "%PHP_PEAR_BIN_DIR%\phpdoc" %*
  GOTO :EOF
)
...
SET count=0
FOR /R "%PHP_PEAR_DATA_DIR%\PhpDocumentor\user" %%I IN (*.ini) DO (
  SET /a count+=1
...
:run
SET found=1
ECHO Starting: "%phpCli%" "%PHP_PEAR_BIN_DIR%\phpdoc" -c "%iniFile%"
ECHO.
"%phpCli%" "%PHP_PEAR_BIN_DIR%\phpdoc" -c "%iniFile%"
GOTO :EOF
